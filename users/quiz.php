<?php 
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST'){
        if ($role == 'student'){
            ...
            $sql = "SELECT question.questID, `option`.optID, question.awardPt FROM question JOIN `option` ON question.questID=`option`.questID WHERE `option`.`IsAnswer`=1";
            $stmt = $conn->prepare($sql);
            $stmt->execute();
            $result = $stmt->get_result();
            $stmt -> close();
        ...
        <div class="page-content">
            ...
            <div class="course-content">
                <form method="post">
                    ...
                    <?php $result->data_seek(0); $count=1;
                    while ($row = $result->fetch_assoc()) { ?>
